...
const schema = {
  ...
  required: ["title", "summary", "markdown", "claims", "sources", "warnings"],
  properties: {
...
function parseArticle(payload: ResponsePayload): ArticleOutput {
  ...
  if (!value.title?.trim() || !value.summary?.trim() || !value.markdown?.trim() || !Array.isArray(value.sources) || !Array.isArray(value.claims) || !Array.isArray(value.warnings)) throw new Error("The AI response did not match the article schema.");
...
export async function generateArticle(source: NewsSource) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if (response.ok) {
      ...
      return { ...article, markdown: `${article.markdown.trim()}\n\n## Sources\n\n${sourceLines}`, sources, model, responseId: payload.id || "", inputTokens: payload.usage?.input_tokens, outputTokens: payload.usage?.output_tokens };
    }
