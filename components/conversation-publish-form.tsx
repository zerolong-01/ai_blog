  ...
  return (
    <form action={action} className="editorForm newsImportForm">
      <label className="fieldGroup">
        <span>글 제목</span>
        <input name="title" required maxLength={200} placeholder="예: 오늘의 AI 소식 — 2026년 9월 18일" readOnly={pending} />
      </label>
      <label className="fieldGroup">
        <span>매일 업데이트된 대화 내용</span>
        <textarea name="sourceText" required minLength={100} maxLength={100_000} rows={18} placeholder="ChatGPT 대화에서 오늘 게시할 업데이트 내용을 복사해 붙여넣으세요. 날짜와 참고 링크도 함께 넣으면 좋습니다." readOnly={pending} />
      </label>
